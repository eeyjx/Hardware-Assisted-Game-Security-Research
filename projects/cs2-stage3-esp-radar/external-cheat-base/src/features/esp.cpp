...
namespace
{
    ...
    ImU32 colorWithAlpha(const ImU32 color, const uint8_t alpha)
    {
        ...
        const float scale)
    {
        ...
        // A very light tint keeps the target readable without covering the
        // game model. The corner-only frame is intentionally less noisy than
        // the old full rectangle when many practice bots are visible.
        drawList->AddRectFilled(
            minimum,
            maximum,
            colorWithAlpha(color, 16),
            2.0f * scale);

        const auto drawCorners = [&](const ImU32 lineColor, const float thickness)
        {
            drawList->AddLine(minimum, ImVec2(minimum.x + corner, minimum.y), lineColor, thickness);
            drawList->AddLine(minimum, ImVec2(minimum.x, minimum.y + corner), lineColor, thickness);
            drawList->AddLine(ImVec2(maximum.x - corner, minimum.y), ImVec2(maximum.x, minimum.y), lineColor, thickness);
            drawList->AddLine(ImVec2(maximum.x, minimum.y), ImVec2(maximum.x, minimum.y + corner), lineColor, thickness);
            drawList->AddLine(ImVec2(minimum.x, maximum.y - corner), ImVec2(minimum.x, maximum.y), lineColor, thickness);
            drawList->AddLine(ImVec2(minimum.x, maximum.y), ImVec2(minimum.x + corner, maximum.y), lineColor, thickness);
            drawList->AddLine(ImVec2(maximum.x - corner, maximum.y), maximum, lineColor, thickness);
            drawList->AddLine(ImVec2(maximum.x, maximum.y - corner), maximum, lineColor, thickness);
        };

        drawCorners(IM_COL32(2, 7, 12, 220), 4.0f * scale);
        drawCorners(color, 2.0f * scale);

        // Small centre ticks give the frame a compact HUD/targeting-reticle
        // identity while keeping the original projected bounds unchanged.
        const float centerX = (minimum.x + maximum.x) * 0.5f;
        const float tick = std::min(5.0f * scale, width * 0.12f);
        drawList->AddLine(
            ImVec2(centerX - tick, minimum.y),
            ImVec2(centerX + tick, minimum.y),
            colorWithAlpha(color, 190),
            1.0f * scale);
    }
...
namespace
{
    ...
    void updateBombInfo()
    {
        ...
        // Local Radar only needs the C4 lifecycle and world position. Reading
        // blow/defuse clocks was exclusive to the removed Bomb Timer.
        bool ticking = false;
        BombInfo sampled{};
        if (!memory::TryRead(
                plantedC4 +
        ...
        if (!ticking && !sampled.hasExploded && !sampled.isDefused) {
            clearBombInfo();
            return;
        }
...
void esp::updateEntities(const menu::RuntimeConfig& config)
{
    ...
    // Cache only the local state needed by ESP and radar rendering.
    const vec3 eyePos = {
        localPos.x + viewOffset.x,
        localPos.y + viewOffset.y,
        localPos.z + viewOffset.z
    };
    {
        std::lock_guard<std::mutex> lock(dataMutex);
        vm = localVm;
    ...
    if (cachedPawns.empty() ||
        ...
        updateNow - lastEntityCacheRefresh >=
            runtime_timing::metadataRefreshInterval(false)) {
    ...
    for (const auto& cp : cachedPawns)
    {
        ...
        // player, but ordinary ESP consumers only use live opponents. Do
        ...
        // recycled during death/respawn cannot become a teammate or stale player
        ...
        // Bone data is sampled only when skeleton ESP is enabled.
    ...
    // Discovery is intentionally slow, while positions are sampled frequently
    // enough to keep moving world markers smooth without rescanning thousands
    // of entity identities every frame.
    ...
    // C4 lifecycle/position is part of the Local Radar snapshot. It is sampled
    // only when a radar frame is due; no countdown clock remains.
    if (!config.radarSnapshotEnabled()) {
        if (lastBombRefresh.time_since_epoch().count() != 0) {
            clearBombInfo();
            lastBombRefresh = {};
        }
    } else if (captureRadarSnapshot) {
...
void esp::render()
{
    ...
    if (menu::espEnabled) {
        ...
        // Build a DPI-aware stack above the frame. Every enabled indicator gets
        // its own vertical slot, including the new padded weapon badge.
        float topCursor = static_cast<float>(y) - 4.0f * scale;
        ImVec2 weaponTextSize{};
        float weaponTextY = topCursor;
        ImVec2 weaponBadgeMinimum{};
        ImVec2 weaponBadgeMaximum{};
        if (menu::espWeapon) {
            weaponTextSize = ImGui::CalcTextSize(enemy.weaponName.c_str());
            const float paddingX = 5.0f * scale;
            const float paddingY = 2.0f * scale;
            const float badgeWidth = weaponTextSize.x + paddingX * 2.0f;
            const float badgeHeight = weaponTextSize.y + paddingY * 2.0f;
            const float centerX = static_cast<float>(x + w / 2);
            weaponBadgeMaximum = ImVec2(
                centerX + badgeWidth * 0.5f,
                topCursor);
            weaponBadgeMinimum = ImVec2(
                centerX - badgeWidth * 0.5f,
                topCursor - badgeHeight);
            weaponTextY = weaponBadgeMinimum.y + paddingY;
            topCursor = weaponBadgeMinimum.y - 3.0f * scale;
        }
        ...
        // Draw a tactical corner frame (color still indicates the original
        // angle-based threat level).
        if (menu::espBox) {
            drawTacticalCornerFrame(
                drawList,
        ...
        if (menu::espFlashIndicator) {
            ...
            // Draw a compact HUD eye: dark outer silhouette, state-coloured
            // iris ring and a bright centre point. The flash-state colour
            // semantics are unchanged.
            const int segments = 32;
            drawList->AddEllipseFilled(
                ImVec2(ellipseCenterX, ellipseCenterY),
                ImVec2(
                    ellipseRadiusX + 1.5f * scale,
                    ellipseRadiusY + 1.5f * scale),
                IM_COL32(2, 7, 12, 225),
                0.0f,
                segments);
            drawList->AddEllipseFilled(
                ImVec2(ellipseCenterX, ellipseCenterY),
                ImVec2(ellipseRadiusX, ellipseRadiusY),
                eyeColor,
                0.0f,
                segments);
            drawList->AddEllipseFilled(
                ImVec2(ellipseCenterX, ellipseCenterY),
                ImVec2(
                    std::max(1.0f, ellipseRadiusX - 1.5f * scale),
                    std::max(1.0f, ellipseRadiusY - 1.5f * scale)),
                IM_COL32(7, 13, 20, 235),
                0.0f,
                segments);
        ...
        // Draw a segmented tactical health rail on the left side.
        if (menu::espHealth) {
            const float healthBarWidth = std::max(3.0f, 4.0f * scale);
            const float healthBarX = static_cast<float>(x) - 9.0f * scale;
            const float healthBarY = static_cast<float>(y);
            const float healthBarHeight = static_cast<float>(h);
            const float rounding = healthBarWidth * 0.5f;

            // Shadow and recessed track.
            drawList->AddRectFilled(
                ImVec2(healthBarX - scale, healthBarY - scale),
                ImVec2(
                    healthBarX + healthBarWidth + scale,
                    healthBarY + healthBarHeight + scale),
                IM_COL32(2, 7, 12, 225),
                rounding + scale);
            drawList->AddRectFilled(
            ...
            const float healthFraction = std::clamp(
                ...
                0.0f,
                1.0f);
            const float healthHeight = healthFraction * healthBarHeight;
            const float healthY = healthBarY + healthBarHeight - healthHeight;
            uint8_t healthR =
                static_cast<uint8_t>(255 * (1.0f - healthFraction));
            uint8_t healthG =
                static_cast<uint8_t>(255 * healthFraction);
            if (healthHeight > 0.0f) {
                drawList->AddRectFilled(
                    ImVec2(healthBarX, healthY),
                    ImVec2(
                        healthBarX + healthBarWidth,
                        healthBarY + healthBarHeight),
                    IM_COL32(healthR, healthG, 38, 255),
                    rounding);
            }

            // Four quiet divisions make the remaining health easier to read
            // at a glance without adding any new value or state.
            for (int segment = 1; segment < 4; ++segment) {
                const float segmentY = healthBarY +
                    healthBarHeight * (static_cast<float>(segment) / 4.0f);
                drawList->AddLine(
                    ImVec2(healthBarX, segmentY),
                    ImVec2(healthBarX + healthBarWidth, segmentY),
                    IM_COL32(4, 10, 15, 180),
                    std::max(1.0f, scale * 0.75f));
            }
        }

        // Draw the current weapon in a compact tactical badge.
        if (menu::espWeapon) {
            ...
            const ImU32 weaponColor = IM_COL32(wr, wg, wb, wa);
            const float badgeRounding = 3.0f * scale;
            drawList->AddRectFilled(
                ImVec2(
                    weaponBadgeMinimum.x + scale,
                    weaponBadgeMinimum.y + scale),
                ImVec2(
                    weaponBadgeMaximum.x + scale,
                    weaponBadgeMaximum.y + scale),
                IM_COL32(1, 5, 9, 190),
                badgeRounding);
            drawList->AddRectFilled(
                weaponBadgeMinimum,
                weaponBadgeMaximum,
                IM_COL32(7, 14, 22, 225),
                badgeRounding);
            drawList->AddRect(
                weaponBadgeMinimum,
                weaponBadgeMaximum,
                colorWithAlpha(weaponColor, 155),
                badgeRounding,
                0,
                std::max(1.0f, scale));
            drawList->AddRectFilled(
                ImVec2(
                    weaponBadgeMinimum.x,
                    weaponBadgeMinimum.y + 2.0f * scale),
                ImVec2(
                    weaponBadgeMinimum.x + 2.0f * scale,
                    weaponBadgeMaximum.y - 2.0f * scale),
                weaponColor,
                scale);
            drawShadowedText(
                drawList,
        ...
        if (menu::espWallCheck) {
            ...
            // Draw a double-layer direction chevron at the top of the frame.
            float centerX = static_cast<float>(x + w / 2);
            float topY = arrowTopY;
            const ImU32 directionColor = IM_COL32(vr, vg, vb, va);

            // Direction still shows whether the enemy faces toward or away
            // from the player; color still reports the spotted-state status.
            if (enemy.angleToPlayer < 90.0f) {
                ...
                drawList->AddTriangleFilled(
                    p1, p2, p3, IM_COL32(2, 7, 12, 230));
                drawList->AddTriangleFilled(
                    ImVec2(centerX, topY + arrowSize - 1.5f * scale),
                    ImVec2(
                        centerX - arrowSize + 2.0f * scale,
                        topY + 1.5f * scale),
                    ImVec2(
                        centerX + arrowSize - 2.0f * scale,
                        topY + 1.5f * scale),
                ...
                drawList->AddTriangleFilled(
                    p1, p2, p3, IM_COL32(2, 7, 12, 230));
                drawList->AddTriangleFilled(
                    ImVec2(centerX, topY + 1.5f * scale),
                    ImVec2(
                        centerX - arrowSize + 2.0f * scale,
                        topY + arrowSize - 1.5f * scale),
                    ImVec2(
                        centerX + arrowSize - 2.0f * scale,
                        topY + arrowSize - 1.5f * scale),
                    directionColor);
        ...
        // Draw distance in a matching lower HUD badge.
        if (menu::espDistance) {
            char distText[32];
            ...
            const ImU32 distanceColor = IM_COL32(dr, dg, db, da);
            const float paddingX = 5.0f * scale;
            const float paddingY = 2.0f * scale;
            const float centerX = static_cast<float>(x + w / 2);
            const ImVec2 badgeMinimum(
                centerX - distanceTextSize.x * 0.5f - paddingX,
                static_cast<float>(y + h) + 3.0f * scale);
            const ImVec2 badgeMaximum(
                centerX + distanceTextSize.x * 0.5f + paddingX,
                badgeMinimum.y + distanceTextSize.y + paddingY * 2.0f);
            drawList->AddRectFilled(
                ImVec2(badgeMinimum.x + scale, badgeMinimum.y + scale),
                ImVec2(badgeMaximum.x + scale, badgeMaximum.y + scale),
                IM_COL32(1, 5, 9, 190),
                3.0f * scale);
            drawList->AddRectFilled(
                badgeMinimum,
                badgeMaximum,
                IM_COL32(7, 14, 22, 220),
                3.0f * scale);
            drawList->AddRect(
                badgeMinimum,
                badgeMaximum,
                colorWithAlpha(distanceColor, 125),
                3.0f * scale,
                0,
                std::max(1.0f, scale * 0.85f));
            drawShadowedText(
                drawList,
                ImVec2(
                    centerX - distanceTextSize.x * 0.5f,
                    badgeMinimum.y + paddingY),
                distanceColor,
                distText,
                scale);
        }
        ...
        if (menu::espSkeleton && enemy.hasBones) {
            ...
            const ImU32 boneGlow = colorWithAlpha(boneColor, 72);
            const ImU32 boneShadow = IM_COL32(2, 7, 12, 225);
        ...
        if (menu::espSnaplines) {
            ...
            drawList->AddLine(
                ImVec2(static_cast<float>(startX), static_cast<float>(startY)),
