...
namespace
{
        ...
        std::string& error)
    {
        ...
        if (!std::filesystem::is_regular_file(path, fileError)) {
            error = "map PNG is missing from local-radar";
            return nullptr;
        }
    ...
    void drawAlivePlayer(
        ...
        const bool local)
    {
        // The new marker is a compact radar contact: a faint direction cone
        // behind a diamond-shaped core. Position, yaw and team colour still
        // come from the exact same snapshot fields as before.
        const std::uint8_t colorAlpha = static_cast<std::uint8_t>(
            (color >> IM_COL32_A_SHIFT) & 0xFFU);
        const std::uint8_t directionAlpha = std::min<std::uint8_t>(
            colorAlpha,
            120U);
        if (yaw && std::isfinite(*yaw)) {
            ...
            const ImVec2 outlineTip(
                center.x + forward.x * radius * 2.55f,
                center.y + forward.y * radius * 2.55f);
            const ImVec2 outlineBase(
                center.x + forward.x * radius * 0.15f,
                center.y + forward.y * radius * 0.15f);
            drawList->AddTriangleFilled(
                outlineTip,
                ImVec2(
                    outlineBase.x + right.x * radius * 0.78f,
                    outlineBase.y + right.y * radius * 0.78f),
                ImVec2(
                    outlineBase.x - right.x * radius * 0.78f,
                    outlineBase.y - right.y * radius * 0.78f),
                IM_COL32(3, 7, 13, 230));
            ...
            const ImVec2 tip(
                center.x + forward.x * radius * 2.28f,
                center.y + forward.y * radius * 2.28f);
            const ImVec2 base(
                center.x + forward.x * radius * 0.25f,
                center.y + forward.y * radius * 0.25f);
            drawList->AddTriangleFilled(
                tip,
                ImVec2(
                    base.x + right.x * radius * 0.56f,
                    base.y + right.y * radius * 0.56f),
                ImVec2(
                    base.x - right.x * radius * 0.56f,
                    base.y - right.y * radius * 0.56f),
                withAlpha(color, directionAlpha));
            drawList->AddLine(
                center,
                tip,
                color,
                std::max(1.0f, radius * 0.16f));
        }

        const auto drawDiamond = [drawList, center](
            const float size,
            const ImU32 diamondColor)
        {
        ...
        drawDiamond(radius + 2.2f, IM_COL32(3, 7, 13, 235));
        drawDiamond(radius + 0.7f, color);
        drawDiamond(radius * 0.54f, IM_COL32(8, 15, 24, 235));
        drawList->AddCircleFilled(
            center,
            std::max(1.2f, radius * 0.22f),
            IM_COL32(239, 247, 255, 245),
            10);
        if (local) {
            drawList->AddCircle(
                center,
                radius + 4.0f,
                IM_COL32(25, 189, 224, 245),
                24,
                2.0f);
            drawList->AddCircle(
                center,
                radius + 6.5f,
                IM_COL32(239, 247, 255, 190),
                24,
                1.0f);
        }
    }
    ...
    void drawDeadPlayer(
        ...
        const float radius)
    {
        const float diamond = radius + 1.5f;
        drawList->AddQuadFilled(
            ImVec2(center.x, center.y - diamond),
            ImVec2(center.x + diamond, center.y),
            ImVec2(center.x, center.y + diamond),
            ImVec2(center.x - diamond, center.y),
            IM_COL32(4, 8, 14, 225));
        const float arm = radius * 0.78f;
        drawList->AddLine(
    ...
    const RenderConfig& config)
{
    ...
    if (texture) {
        ...
        drawList->AddImage(
            ...
            minimum,
            maximum);
        // A restrained blue-black tint ties the map texture into the HUD
        // palette without changing projection or hiding map geometry.
        drawList->AddRectFilled(
            minimum,
            maximum,
            IM_COL32(3, 10, 22, 24));
    ...
    // Quiet quarter-grid lines improve positional scanning. They are purely
    // visual and use the same unmodified map rectangle as player projection.
    for (int gridLine = 1; gridLine < 4; ++gridLine) {
        const float fraction = static_cast<float>(gridLine) / 4.0f;
        const float gridX = minimum.x + side * fraction;
        const float gridY = minimum.y + side * fraction;
        const ImU32 gridColor = gridLine == 2
            ? IM_COL32(25, 189, 224, 42)
            : IM_COL32(124, 97, 245, 24);
        drawList->AddLine(
            ImVec2(gridX, minimum.y),
            ImVec2(gridX, maximum.y),
            gridColor,
            1.0f);
        drawList->AddLine(
            ImVec2(minimum.x, gridY),
            ImVec2(maximum.x, gridY),
            gridColor,
            1.0f);
    }
    ...
    for (const game::PlayerSnapshot& player : snapshot.players) {
        ...
        if (levelIndex) {
            ...
            if (floorMarker) {
                const ImVec2 floorTextSize = ImGui::CalcTextSize(floorMarker);
                const ImVec2 floorMinimum(
                    center.x + markerRadius + 2.0f,
                    center.y - markerRadius - 4.0f);
                const ImVec2 floorMaximum(
                    floorMinimum.x + floorTextSize.x + 4.0f,
                    floorMinimum.y + floorTextSize.y + 2.0f);
                drawList->AddRectFilled(
                    floorMinimum,
                    floorMaximum,
                    IM_COL32(5, 10, 18, 220),
                    2.0f);
                drawList->AddRect(
                    floorMinimum,
                    floorMaximum,
                    IM_COL32(25, 189, 224, 150),
                    2.0f);
                drawList->AddText(
                    ImVec2(
                        floorMinimum.x + 2.0f,
                        floorMinimum.y + 1.0f),
                    IM_COL32(239, 247, 255, 255),
                    floorMarker);
            }
        }
        ...
        if (config.showNames && !player.name.empty()) {
            ...
            const float badgeWidth = nameSize.x + paddingX * 2.0f;
            const float badgeHeight = nameSize.y + paddingY * 2.0f;
            const float badgeX = std::clamp(
                center.x - badgeWidth * 0.5f,
                minimum.x + 2.0f,
                std::max(
                    minimum.x + 2.0f,
                    maximum.x - badgeWidth - 2.0f));
            const float badgeY = center.y + markerRadius + 4.0f;
            const ImVec2 badgeMinimum(badgeX, badgeY);
            const ImVec2 badgeMaximum(
                badgeX + badgeWidth,
                badgeY + badgeHeight);
            drawList->AddRectFilled(
                ImVec2(badgeMinimum.x + 1.0f, badgeMinimum.y + 1.0f),
                ImVec2(badgeMaximum.x + 1.0f, badgeMaximum.y + 1.0f),
                IM_COL32(1, 4, 9, 170),
                3.0f);
            drawList->AddRectFilled(
                badgeMinimum,
                badgeMaximum,
                IM_COL32(6, 12, 21, 215),
                3.0f);
            drawList->AddRect(
                badgeMinimum,
                badgeMaximum,
                ...
                3.0f);
            drawList->AddRectFilled(
                ImVec2(badgeMinimum.x, badgeMinimum.y + 2.0f),
                ImVec2(badgeMinimum.x + 2.0f, badgeMaximum.y - 2.0f),
                color,
                1.0f);
            drawList->AddText(
                ImGui::GetFont(),
                fontSize,
                ImVec2(
                    badgeMinimum.x + paddingX + 1.0f,
                    badgeMinimum.y + paddingY + 1.0f),
                IM_COL32(1, 5, 10, 220),
                player.name.c_str());
            drawList->AddText(
                ImGui::GetFont(),
    ...
    if (bombVisible) {
        ...
        if (position) {
            ...
            if (point.valid && point.inside) {
                ...
                drawList->AddQuadFilled(
                    ImVec2(center.x, center.y - radius - 2.0f),
                ...
                drawList->AddQuadFilled(
                    ...
                    ImVec2(center.x, center.y + radius + 0.8f),
                    ImVec2(center.x - radius - 0.8f, center.y),
                    IM_COL32(255, 94, 120, 105));
                drawList->AddQuadFilled(
                    ImVec2(center.x, center.y - radius),
                    ImVec2(center.x + radius, center.y),
                    ImVec2(center.x, center.y + radius),
                    ImVec2(center.x - radius, center.y),
                    IM_COL32(255, 94, 120, 255));
                drawList->AddText(
                    ImVec2(
                        center.x + radius + 3.0f,
                        center.y - radius + 1.0f),
                    IM_COL32(2, 6, 11, 225),
                    "C4");
                drawList->AddText(
                    ImVec2(center.x + radius + 2.0f, center.y - radius),
                    IM_COL32(255, 214, 224, 255),
                    "C4");
            }
        }
    }
    ...
    drawList->AddRectFilled(
        minimum,
        ImVec2(minimum.x + 3.0f, minimum.y + 22.0f),
        IM_COL32(25, 189, 224, 245));
    drawList->AddLine(
        ImVec2(minimum.x, minimum.y + 22.0f),
        ImVec2(maximum.x, minimum.y + 22.0f),
        IM_COL32(124, 97, 245, 125),
        1.0f);
    drawList->AddText(
        ImVec2(minimum.x + 7.0f, minimum.y + 3.0f),
        IM_COL32(25, 189, 224, 255),
        "N ^");
    ...
    drawList->AddRect(
        minimum,
        maximum,
        IM_COL32(25, 189, 224, 190),
        0.0f,
        0,
        1.25f);

    // Cyan and violet corner locators finish the shared tactical HUD style.
    const float corner = std::clamp(side * 0.055f, 10.0f, 22.0f);
    const ImU32 cyan = IM_COL32(25, 189, 224, 245);
    const ImU32 violet = IM_COL32(140, 97, 245, 245);
    drawList->AddLine(minimum, ImVec2(minimum.x + corner, minimum.y), cyan, 2.0f);
    drawList->AddLine(minimum, ImVec2(minimum.x, minimum.y + corner), cyan, 2.0f);
    drawList->AddLine(
        ImVec2(maximum.x - corner, minimum.y),
        ImVec2(maximum.x, minimum.y),
        violet,
        2.0f);
    drawList->AddLine(
        ImVec2(maximum.x, minimum.y),
        ImVec2(maximum.x, minimum.y + corner),
        violet,
        2.0f);
    drawList->AddLine(
        ImVec2(minimum.x, maximum.y - corner),
        ImVec2(minimum.x, maximum.y),
        violet,
        2.0f);
    drawList->AddLine(
        ImVec2(minimum.x, maximum.y),
        ImVec2(minimum.x + corner, maximum.y),
        violet,
        2.0f);
    drawList->AddLine(
        ImVec2(maximum.x - corner, maximum.y),
        maximum,
        cyan,
        2.0f);
    drawList->AddLine(
        ImVec2(maximum.x, maximum.y - corner),
        maximum,
        cyan,
        2.0f);
}
